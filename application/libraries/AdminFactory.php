...
public function getAdminTypecache($id = 0,$admin_id=0) {
   if ($id > 0) {
...
/////////////cache////////////
        ...
        $time_cach_set_min=$this->_ci->config->item('time_cach_set_min');
        $time_cach_set=$this->_ci->config->item('time_cach_set');
        $time_cach_level0=$this->_ci->config->item('time_cach_level0');
    	   $timecache=$time_cach_set;
        $lang=$this->_ci->lang->line('lang'); 
        $langs=$this->_ci->lang->line('langs'); 
        $lang='th';
    	   $key='getAdminType_'.$id.'_'.$admin_id;
        if(!$data_cach=$this->_ci->cache->get($key)){
   #####################
   ...
   $data_all=$query->result_object();
   $row=$query->row();
   if($num_rows>0){ 
      $data_cach=$row;  
      $this->_ci->cache->file->save($key,$data_cach,$timecache);
    }else{ 
      $data_cach=false;
    }
   #####################
    // echo '<pre>$sql=>'; print_r($sql); echo '</pre>'; #Die();
    // echo '<pre>$num_rows=>'; print_r($num_rows); echo '</pre>'; #Die();
    // echo '<pre>$row=>'; print_r($row); echo '</pre>'; #Die();
   }
/////////////cache////////////  
    return $data_cach; 
...
/////////////cache////////////
        ...
        $langs=$this->_ci->lang->line('langs'); 
        $lang='th';
    	   $key='getAdminType_by'.$orderby.'_ASC';
        if(!$data_cach=$this->_ci->cache->get($key)){
################ 
...
/////////////cache////////////
        ...
        $langs=$this->_ci->lang->line('langs'); 
        $lang='th';
    	   $key='getAdminType_by'.$orderby.'_up2';
        if(!$data_cach=$this->_ci->cache->get($key)){
################ 
...
/////////////cache////////////
        ...
        $langs=$this->_ci->lang->line('langs'); 
        $lang='th';
    	   $key='getAdminType_order_by'.$orderby.'_asc';
        if(!$data_cach=$this->_ci->cache->get($key)){
################ 
...
public function getUserTypena($id = 0, $admin_type = 0) {

    	if ($id > 0){

			$sql = 'SELECT at.*, aam.`user_access_id`, aam.`user_menu_id` AS access_menu, am.`user_menu_id2`, am.`title`,am.`parent` 
			FROM `sd_user_type` AS `at`
			LEFT JOIN `sd_user_access_menu` aam ON (at.`user_type_id`=aam.`user_type_id` AND at.`status`=1)
			LEFT JOIN `sd_user_menu` am ON (aam.`user_menu_id`= am.`user_menu_id2` AND am.`status`=1)
			WHERE at.`user_type_id` = '.$id;
			 echo $sql;
			...
			if($user_type == 0)
	    		$query = $this->_ci->db->select("*")->from("_user_type")->order_by($orderby, 'ASC')->get();
			else
	    		$query = $this->_ci->db->select("*")->from("_user_type")->where("user_type_id >=", $user_type)->order_by($orderby, 'ASC')->get();
...
public function getAdminType1($id = 0,$admin_id=0) {
    	if ($id > 0) {
    		...
    		return false;
               ...
               $user_type=$this->_ci->session->userdata('user_type');
              //echo '$user_type='.$user_type;Die();
              //echo '$user_id='.$user_id;Die();
    		    //Getting all the users
			$orderby = "user_type_id";
$queryna1 = $this->_ci->db->select("*")->from("_user_type")->where($orderby.'>', '2')->order_by($orderby, 'ASC')->get();
$queryna2 = $this->_ci->db->select("*")->from("_user_type")->order_by($orderby, 'ASC')->order_by($orderby, 'ASC')->get();
...
public function status_admin($id, $enable = 1){
